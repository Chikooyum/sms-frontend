...
<template>
  <v-card>
    ...
    <div v-if="reportData" class="pa-4">
      ...
      <!-- Bagian baru: Daftar siswa yang tidak hadir -->
        ...
        <v-col cols="12" md="4">
          <v-card variant="outlined" class="h-100">
            <v-card-title class="text-h6 text-warning">
              ...
              Siswa Sakit ({{ reportData.sakit_list?.length || 0 }})
            </v-card-title>
            ...
            <v-card-text v-else class="text-center text-grey py-8">
              ...
              <div>Tidak ada siswa yang sakit</div>
            </v-card-text>
          </v-card>
        </v-col>
        ...
        <v-col cols="12" md="4">
          <v-card variant="outlined" class="h-100">
            <v-card-title class="text-h6 text-info">
              ...
              Siswa Izin ({{ reportData.izin_list?.length || 0 }})
            </v-card-title>
            ...
            <v-card-text v-else class="text-center text-grey py-8">
              <v-icon size="48" class="mb-2">mdi-check-circle</v-icon>
              <div>Tidak ada siswa yang izin</div>
            </v-card-text>
          </v-card>
        </v-col>
        ...
        <v-col cols="12" md="4">
          <v-card variant="outlined" class="h-100">
            <v-card-title class="text-h6 text-error">
              ...
              Siswa Alpa ({{ reportData.alpa_list?.length || 0 }})
            </v-card-title>
            ...
            <v-card-text v-else class="text-center text-grey py-8">
              <v-icon size="48" class="mb-2">mdi-check-circle</v-icon>
              <div>Tidak ada siswa yang alpa</div>
            </v-card-text>
          </v-card>
        </v-col>
      </v-row>

      <!-- Detail per kelas (existing functionality) -->
      <div class="mt-4" v-if="reportData.details_by_class?.length > 0">
        <v-card variant="tonal">
          <v-card-title class="text-h6">Detail per Kelas</v-card-title>
          <v-card-text>
            <v-expansion-panels variant="accordion">
              <v-expansion-panel
                v-for="classDetail in reportData.details_by_class"
                :key="classDetail.class_name"
              >
                <v-expansion-panel-title>{{ classDetail.class_name }}</v-expansion-panel-title>
                <v-expansion-panel-text>
                  <v-table density="compact">
                    ...
                    <tbody>
                      <tr
                        v-for="record in classDetail.attendance_records"
